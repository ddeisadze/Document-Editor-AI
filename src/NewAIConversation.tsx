        ...
        >
          <GridItem pl="2" area={"header"}>
            <IconButton
              ...
              margin={"5px"}
              aria-label="Search database"
              icon={<MinusIcon onClick={() => setMinimized(!isMinimized)} />}
            />
          </GridItem>
          <GridItem pl="2" area={"main"} maxHeight="500px">
            <ChatContainer>
              <MessageList
                typingIndicator={
                  openAiLoading && <TypingIndicator content="AiDox is typing" />
                }
              >
                {chatState.map((item: any) => (
                  <Message model={item} />
                ))}
                {chatState[chatState.length - 1].message
                  ?.toLowerCase()
                  ...
                  >
                    <Message.CustomContent>
                      <Button
                        onClick={(e) =>
                          props.handleUpdatePrompt?.call({}, aiAnswer)
                        }
                      >
                        Add generated prompt to Document
                      </Button>
                    </Message.CustomContent>
                  </Message>
                )}
